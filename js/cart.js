...
function renderCart(orderDetail, resJson) {
  ...
  let currentCheck =
    String(orderDetail.id) + orderDetail.colorCode + orderDetail.size;

  let contentWrap = document.getElementById("contentWrap");
  let totalPrice = document.getElementById("total__price");
  let finalPrice = document.getElementById("final__price");
  let freightFee = document.getElementById("total__delivery");
  ...
  delImg.src = "img/cart-remove.png";
  ...
  priceInfo.className = "price__info";
  ...
  for (i in resJson.data.variants) {
    // get current stock of product
    if (
      orderDetail.size === resJson.data.variants[i].size &&
      orderDetail.colorCode === resJson.data.variants[i].color_code
    ) {
...
function updateQty(currentCheckProduct) {
  ...
  for (i in orderList) {
    let orderListCheck =
      String(orderList[i].id) + orderList[i].color.code + orderList[i].size;
...
function emptyCart() {
  ...
  contentWrap.setAttribute(
    "style",
    "text-align: center; padding: 100px 0 100px 0; font-size:16px"
  );

  document.getElementById("total__price").innerHTML = "NT. 0";
  document.getElementById("total__delivery").innerHTML = "NT. 0";
  document.getElementById("final__price").innerHTML = "NT. 0";
}
...
function CheckData() {
  ...
  let shipping = document.getElementById("delivery__country--select").value; //  choosing <select> can get <option> value automaticlly
  let payment = document.getElementById("pay__type--select").value;
...
function CheckOutAPI(data) {
  ...
  fetch(src, {
    ...
    .then(function (resJson) {
      // console.log(resJson);
      localStorage.removeItem("list");
      window.location = "thankyou.html?orderNumber=" + resJson.data.number;
